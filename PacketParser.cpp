...
void PacketParser::call_parser(vector<string> packet_lines)
{
	for (int i = 0; i < packet_lines.size(); i++)
	{
		parse_packet_input(i, packet_lines[i]);
	}
}
...
map<string, string> PacketParser::parse_packet_input(int packet_line_number, string packet_input)
{
	...
	packet_info["data_field"] = packet_input.substr(TYPE_START * BYTE_IN_HEXA, line_length -(TOTAL_LENGTH_WITHOUT_DATA * BYTE_IN_HEXA) );
	return packet_info;
}
...
Packet PacketParser::create_packet(map<string, string> packet_info)
{
	...
	{
		map<string, string>eCPRI_packet_info = parse_packet_data(packet_info.find("data_field")->second);
		eCPRIPacket(packet_info, eCPRI_packet_info);
		// return child w el type parent 
